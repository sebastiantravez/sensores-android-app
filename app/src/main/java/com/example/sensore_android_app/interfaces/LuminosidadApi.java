...
import static com.example.sensore_android_app.utils.Const.BAR_LUMINOSIDAD_NAME_2;
import static com.example.sensore_android_app.utils.Const.LINE_LUMINOSIDAD_NAME;
import static com.example.sensore_android_app.utils.Const.LINE_LUMINOSIDAD_NAME_2;
...
public interface LuminosidadApi {
    ...
    @GET(BAR_LUMINOSIDAD_NAME_2 + "/data")
    Call<Luminosidad> getLuminosidad2(@Header("X-Auth-Token") String token,
                                     @Query("tz") String timeZone,
                                     @Query("start") String start,
                                     @Query("end") String end);

    @GET(LINE_LUMINOSIDAD_NAME_2 + "/data")
    Call<LuminosidadTable> getLuminosidadTable2(@Header("X-Auth-Token") String token,
                                               @Query("tz") String timeZone,
                                               @Query("start") String start,
                                               @Query("end") String end);
}
